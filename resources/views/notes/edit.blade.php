...
<div class="max-w-4xl mx-auto">
    <div class="mb-6">
        ...
        <p class="text-gray-600">Atualize o conteúdo da sua anotação.</p>
    </div>
        ...
        <form action="{{ route('notes.update', $note) }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <textarea name="content" id="content" rows="12"
                    class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">{{ old('content', $note->content) }}</textarea>
                @error('content')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            <div class="flex justify-end space-x-4">
                <a href="{{ route('notes.show', $note) }}" class="px-4 py-2 text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-lg">Cancelar</a>
